...
import { useUserStore } from "./user-store"; // Adjust path if needed
const baseUrl = "http://localhost:3000"; // Replace with your actual backend base URL

export const useCompanyStore = defineStore("company", {
  // Fixed: changed from 'job' to 'company'
  state: () => ({
    company: null,
    loading: false,
    error: null,
  ...
  actions: {
    async fetchCompanyByRecruiter() {
      ...
      try {
        console.log(
          "Making request to:",
          `${baseUrl}/company/getCompanyByRecruiter`
        );
        ...
        if (!res.ok) {
          ...
          throw new Error(
            errorData.message || `HTTP ${res.status}: Failed to fetch company`
          );
        }
    ...
    async updateCompany(companyData) {
      ...
      try {
        ...
        if (!res.ok) {
          const errorData = await res.json();
          throw new Error(
            errorData.message || `HTTP ${res.status}: Failed to update company`
          );
        }
